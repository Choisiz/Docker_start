...
1. 도커파일을 작성한다.
2. 도커클라이언트에게 도커파일을 전달한다.
   - ex) docker build -t <원하는 이미지 이름> .
3. 도커클라이언트가 도커서버에 요청
4. 이미지가 생성된다
...
7. 웹서버(nginx)만들어 리액트 돌리기

- 개발환경: 리액트컨테이너 구성(개발서버, index.html, js&css파일 등등)
- 개발환경: 개발서버를 통해서 브라우저랑 통신
- 운영환경: 리액트컨테이너에(개발서버가 없음)
- build하면 정적파일이 생성되며 로컬에서는 로컬브라우저로 정적파일을 실해하기 때문에 동작
- 그러나 도커는 리액트도커컨테이너와 브라우저간 환경이 다르기때문에, 즉 컨테이너는 외부와 독립적인 환경이므로 정적파일만
  있으며 직접적으로 실행할수없다. 그렇기 때문에 실행시켜줄 웹서버 nginx를 통해 외부에서 도커컨테이너를 통해 접근가능하게 해야한다.
